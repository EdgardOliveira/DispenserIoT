...
export default function ListaUsuarios({usuarios} : any) {
                    ...
                    <TableBody>
                        {rows.map((row) => (
                            <TableRow key={row.id}>
                                ...
                                <TableCell>{row.primeiroNome}</TableCell>
                                <TableCell>{row.ultimoNome}</TableCell>
...
ListaUsuarios.getInitialProps = async (ctx: NextPageContext) => {
    let url = !process.env.BASE_URL ? '/api/usuarios' : `${process.env.BASE_URL}/api/usuarios`;
